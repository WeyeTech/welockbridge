...
class ExampleActivity : AppCompatActivity() {
    ...
    private fun connectToDevice(scannedDevice: ScannedDevice) {
        ...
        lifecycleScope.launch {
            // Determine which protocol to use based on device name
            val deviceName = scannedDevice.name?.lowercase() ?: ""
            val useTTSeries = deviceName.contains("tt") ||
                    deviceName.contains("elock") ||
                    deviceName.contains("totarget")

            val credentials = if (useTTSeries) {
                // TT-Series credentials
                Log.d(TAG, "Using TT-Series protocol")
                detectedProtocol = LockProtocol.TT_SERIES
                DeviceCredentials.forTTSeries(
                    lockId = "00000000",  // Will auto-detect from device
                    password = "112233"   // Replace with actual password
